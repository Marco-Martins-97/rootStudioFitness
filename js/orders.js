function loadOrders(){
    $.post('includes/loadServerData.inc.php', {action: 'loadOrders'}, function(response){
        ...
        if (response.status === 'error') {
            console.warn('Erro no servidor:', response.message || 'Erro desconhecido');
            $('.orders-container').html('Ocorreu um erro. Não foi possível carregar as encomendas!');
            return;
        }
        ...
        if(ordersProducts.length > 0){
            ...
            // Substitui o status pelo equivalente em português
...
function reviewOrder(orderId, review){
    $.post('includes/saveServerData.inc.php', {action: 'reviewOrder', orderId: orderId, review: review}, function(response){
        if (response.status === 'error') {
            console.error('Erro no servidor:', response.message || 'Erro desconhecido');
